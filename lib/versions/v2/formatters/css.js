...
Formatter.CSS = Formatter.create(
	{
		...
		prototype: {
			format: function(contents, lineOffset) {
				...
				instance._extractRecursive(contents, '{', '}', instance._checkPadding.bind(this));
			...
			_checkPadding: function(item) {
				var instance = this;

				var lineStart = item.lineStart;
				var str = item.str;

				if (!str || str.indexOf('content-box') > -1) {
					return;
				}
			...
			_extractRecursive: function(str, open, close, cb) {
				var instance = this;
